...
use Expression::{Number, Boolean, Add, Multiply, LessThan, Variable};
...
fn reduce_variable(name: String, env: &Environment) -> Expression {
    env.get(&name).expect(&format!("variable {} does not exist", name)).clone()
}

#[cfg(test)]
mod tests {
    use std::collections::HashMap;
    use super::Expression::{Number, Boolean, Add, Multiply, LessThan, Variable};
    use super::{Expression, Environment, reduce_add, reduce_lessthan, reduce_multiply, reduce_variable};
    ...
    fn test_reduce_add() {
        ...
        let b = Number(11);
        assert_eq!(reduce_add(a.clone(), b.clone(), &env), Expression::new_add(3, 11));
        assert_eq!(reduce_add(b.clone(), a.clone(), &env), Expression::new_add(11, 3));
    }
    ...
    fn test_reduce_multiply() {
        ...
        let b = Number(11);
        assert_eq!(reduce_multiply(a.clone(), b.clone(), &env), Expression::new_multiply(6, 11));
        assert_eq!(reduce_multiply(b.clone(), a.clone(), &env), Expression::new_multiply(11, 6));
    }
